...
| controller | 4D.Class | -> | Controller class to attach (default: `cs.ppt_rs._CLI_Controller`) |

## Description

`_CLI` resolves the platform-specific path to a bundled executable and attaches a controller that manages the underlying `4D.SystemWorker`. It is extended by [`pptcli`](pptcli.md) and should not be instantiated directly.
...
| controller | cs.ppt_rs._CLI_Controller | Attached controller instance |
...
- [`pptcli`](pptcli.md) — extends `_CLI` to wrap the `pptcli` executable
