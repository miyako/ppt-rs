...
> _CLI_Controller.new (CLI : cs.ppt_rs._CLI)
...
| instance | cs.ppt_rs._CLI | The owning `_CLI` instance |
...
#### execute (command, message, context) → cs.ppt_rs._CLI_Controller
...
| Result | cs.ppt_rs._CLI_Controller | <- | `This` — enables chaining |
...
- [`_pptcli_Controller`](_pptcli_Controller.md) — extends `_CLI_Controller` with stdout/stderr accumulation and progress parsing
